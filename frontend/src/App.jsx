                ...
                <div className="bg-white border-2 border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      ...
                      {validationResult?.valid ? 'Re-validate' : 'Validate'}
                    </button>
                  </div>
                  {validationResult?.valid && (
                  ...
                  <div className="bg-white border-2 border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex gap-3">
                        ...
                        >
                          {loadingPRStatus ? <Loader className="animate-spin" size={20} /> : <RefreshCw size={20} />}
                          Check Status
                        </button>
                        
                        {/* Show Create PR button if no PR exists or PR is closed */}
                        {(!prStatus || (!prStatus.pr_exists || (prStatus.pr_state === 'closed' && !prStatus.is_merged))) && (
                          <button
                            onClick={handleCreatePR}
                            disabled={loadingCreatePR}
                            className="px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                          >
                            {loadingCreatePR ? <Loader className="animate-spin" size={20} /> : <GitPullRequest size={20} />}
                            Create PR
                          </button>
                        )}
                      </div>
